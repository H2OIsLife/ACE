...
text(matrix(c(0.3,0.45),nrow=1),labels="Stress=0.1387")
...
text(matrix(c(-0.6,0.17),nrow=1),labels="Stress=0.194")
...
text(matrix(c(-0.9,0.2),nrow=1),labels="Stress=0.2104")
...
text(matrix(c(-1.1,0.3),nrow=1),labels="Stress=0.1359")
...
text(matrix(c(-0.95,0.15),nrow=1),labels="Stress=0.1125")

```

RDA-before treatments are separated
...
names(maumcounts)
maum.colsums=apply(maumcounts[,4:70],2,sum)
maum.rowsums=apply(maumcounts[,4:70],1,sum)
min(maum.rowsums)
min=min(maum.rowsums)
n=nrow(maumcounts)

# Rarefaction and Hellinger distance transformation
maum.rare=rrarefy(maumcounts[,4:70],sample=min)
str(maum.rare)
maum.rare.hel=cbind(maumcounts[,1:3],decostand(maum.rare,method="hellinger"))
maum.stands.hel.dist=vegdist(maum.rare.hel[4:70],method="euclidean")
#^when is this used? Everything just uses maum.rare.hel.... not the actual distance matrix..

# RDA - Redundancy Analysis 
rda.maum.rare.hel.all=rda(maum.rare.hel[,4:70]~maum.rare.hel$Treatment)
	summary(rda.maum.rare.hel.all)
	anova(rda.maum.rare.hel.all)
	RsquareAdj(rda.maum.rare.hel.all)

	anova(rda.maum.rare.hel.all,by="terms")

#should I separate out treatment types??/
...
# Here is the one-line formula interface method.
	anova(rda.maum.rare.hel.all,by="margin")
...
rda.maum.rare.hel.int=rda(maum.rare.hel[,4:70] ~ maum.rare.hel$Treatment)
	anova(rda.maum.rare.hel.int)
...
plot(rda.maum.rare.hel.all)
# The above plot was not so great.
# Better plots can be made if you extract scores as shown below
rda.maum.rare.hel.all.spscores=scores(rda.maum.rare.hel.all,display="sp",choices=c(1:17))
head(rda.maum.rare.hel.all.spscores)
rda.maum.rare.hel.all.lcscores=scores(rda.maum.rare.hel.all,display="lc",choices=c(1:17))
head(rda.maum.rare.hel.all.lcscores)
rda.maum.rare.hel.all.wascores=scores(rda.maum.rare.hel.all,display="wa",choices=c(1:17))
head(rda.maum.rare.hel.all.wascores)


#modify for treatments colors=ifelse(maum.rare.hel$Treatment=="Leaf",3,6)
#symbols=ifelse(maum.rare.hel$Treatment=="BOWO",1,ifelse(maum.rare.hel$Ecosys=="SMBW",2,0))

#weighted averages
plot(rda.maum.rare.hel.all.wascores[,1:2],col=colors,xlab="RDA1 (14%)",ylab="RDA2 (9%)")
points(rda.maum.rare.hel.all.lcscores[,1:2],col=colors,cex=2,lwd=4)
#linear contrasts
plot(rda.maum.rare.hel.all.lcscores[,1:2],col=colors,xlab="RDA1 (14%)",ylab="RDA2 (9%)")
points(rda.maum.rare.hel.all.lcscores[,1:2],col=colors,cex=2,lwd=4)
#species scores
plot(rda.maum.rare.hel.all.spscores[,1:2],col=colors,xlab="RDA1 (14%)",ylab="RDA2 (9%)")
points(rda.maum.rare.hel.all.spscores[,1:2],col=colors,cex=2,lwd=4)

# This command provides and R-squared for each taxon for each axis
goodness(rda.maum.rare.hel.all)
sp.rsq=goodness(rda.maum.rare.hel.all)
sp.aff=rownames(sp.rsq)[sp.rsq[,2]>0.3]
sp.aff.rows=match(sp.aff,rownames(rda.maum.rare.hel.all.spscores))

text(rda.maum.rare.hel.all.spscores[sp.aff.rows,1:2],labels=rownames(rda.maum.rare.hel.all.spscores[sp.aff.rows,]))

```
RDA post treatment separation
```{r}
#new vegan update installed
library(vegan)
...
maum.stands.hel.dist=vegdist(maum.rare.hel[4:70],method="euclidean")
maum.rare.hel=cbind(maumcounts[,71:73],maum.rare.hel1)
str(maum.rare.hel)

# RDA - Redundancy Analysis -Treatments apart
rda.maum.rare.hel.all.treats=rda(maum.rare.hel[,7:73]~maum.rare.hel$trace*maum.rare.hel$nitrogen*maum.rare.hel$phosphorus)
	summary(rda.maum.rare.hel.all.treats)
	anova(rda.maum.rare.hel.all.treats)
	RsquareAdj(rda.maum.rare.hel.all.treats)
		anova(rda.maum.rare.hel.all.treats,by="terms")
	anova(rda.maum.rare.hel.all.treats,by="margin")
...
text(0.2,0.6,"Rsquared adj.= 14.4%")
# symbols N
...
sp.rsq=goodness(rda.maum.rare.hel.all.treats)
sp.aff=rownames(sp.rsq)[sp.rsq[,2]>0.3]
sp.aff.rows=match(sp.aff,rownames(rda.maum.rare.hel.all.treats))
#^yielding NAs....why?
text(rda.maum.rare.hel.all.treats[sp.aff.rows,1:2],labels=rownames(rda.maum.rare.hel.all.treats[sp.aff.rows,]))
...
dim(necounts)
names(necounts)
ne.colsums=apply(necounts[,5:147],2,sum)
ne.rowsums=apply(necounts[,5:147],1,sum)
min(ne.rowsums)
min=min(ne.rowsums)
n=nrow(necounts)

# Rarefaction and Hellinger distance transformation
ne.rare=rrarefy(necounts[,5:147],sample=min)
str(ne.rare)
...
ne.rare.hel=cbind(necounts[,148:150],ne.rare.hel1)
str(ne.rare.hel)

# RDA - Redundancy Analysis -Treatments apart
rda.ne.rare.hel.all.treats=rda(ne.rare.hel[,8:150]~ne.rare.hel$trace*ne.rare.hel$nitrogen*ne.rare.hel$phosphorus)
	summary(rda.ne.rare.hel.all.treats)
	anova(rda.ne.rare.hel.all.treats)
	RsquareAdj(rda.ne.rare.hel.all.treats)
		anova(rda.ne.rare.hel.all.treats,by="terms")
	anova(rda.ne.rare.hel.all.treats,by="margin")
...
plot(rda.ne.rare.hel.all.wascores[,1:2],col=colors,xlab="RDA1 (14.3%)",ylab="RDA2 (7.3%)",main="North Erie Community, Hellinger transformed")
points(rda.ne.rare.hel.all.lcscores[,1:2],col=colors,pch=symbols,cex=2,lwd=4)
text(0.3,-0.8,"Rsquared adj.= 20.1%")
# This command provides and R-squared for each taxon for each axis
goodness(rda.ne.rare.hel.all.treats)

sp.rsq=goodness(rda.ne.rare.hel.all.treats)
sp.aff=rownames(sp.rsq)[sp.rsq[,2]>0.3]
sp.aff.rows=match(sp.aff,rownames(rda.ne.rare.hel.all.treats))
#^yielding NAs....why?
text(rda.ne.rare.hel.all.treats[sp.aff.rows,1:2],labels=rownames(rda.ne.rare.hel.all.treats[sp.aff.rows,]))
```
GREEN BAY RDA
```{r}
#add columns for treatments n,p,trace as y/n for analysis of interactive effects
...
rda.gb.rare.hel.all.treats=rda(gb.rare.hel[,8:55]~gb.rare.hel$trace*gb.rare.hel$nitrogen*gb.rare.hel$phosphorus)
	...
	RsquareAdj(rda.gb.rare.hel.all.treats)
		anova(rda.gb.rare.hel.all.treats,by="terms")
	anova(rda.gb.rare.hel.all.treats,by="margin")
...
text(0.2,-0.5,"Rsquared adj.= 9.8%")
# This command provides and R-squared for each taxon for each axis
goodness(rda.gb.rare.hel.all.treats)

sp.rsq=goodness(rda.gb.rare.hel.all.treats)
sp.aff=rownames(sp.rsq)[sp.rsq[,2]>0.3]
sp.aff.rows=match(sp.aff,rownames(rda.gb.rare.hel.all.treats))
#^yielding NAs....why?
text(rda.gb.rare.hel.all.treats[sp.aff.rows,1:2],labels=rownames(rda.gb.rare.hel.all.treats[sp.aff.rows,]))
```
FORD RDA
```{r}
#add columns for treatments n,p,trace as y/n for analysis of interactive effects
...
dim(fordcounts)
names(fordcounts)
ford.colsums=apply(fordcounts[,4:80],2,sum)
ford.rowsums=apply(fordcounts[,4:80],1,sum)
min(ford.rowsums)
min=min(ford.rowsums)
n=nrow(fordcounts)

# Rarefaction and Hellinger distance transformation
ford.rare=rrarefy(fordcounts[,4:80],sample=min)
str(ford.rare)
ford.rare.hel1=cbind(fordcounts[,1:3],decostand(ford.rare,method="hellinger"))
#ne.stands.hel.dist=vegdist(ne.rare.hel[5:147],method="euclidean")
ford.rare.hel=cbind(fordcounts[,81:83],ford.rare.hel1)
str(ford.rare.hel)

# RDA - Redundancy Analysis -Treatments apart
rda.ford.rare.hel.all.treats=rda(ford.rare.hel[,7:83]~ford.rare.hel$trace*ford.rare.hel$nitrogen*ford.rare.hel$phosphorus)
	summary(rda.ford.rare.hel.all.treats)
	anova(rda.ford.rare.hel.all.treats)
	RsquareAdj(rda.ford.rare.hel.all.treats)
		anova(rda.ford.rare.hel.all.treats,by="terms")
	anova(rda.ford.rare.hel.all.treats,by="margin")
...
plot(rda.ford.rare.hel.all.wascores[,1:2],col=colors,xlab="RDA1 (18.9%)",ylab="RDA2 (12.3%)",main="Ford Community, Hellinger transformed")
points(rda.ford.rare.hel.all.lcscores[,1:2],col=colors,pch=symbols,cex=2,lwd=4)
text(-0.5,0.5,"Rsquared adj.= 33.6%")
# This command provides and R-squared for each taxon for each axis
goodness(rda.ford.rare.hel.all.treats)

sp.rsq=goodness(rda.ford.rare.hel.all.treats)
sp.aff=rownames(sp.rsq)[sp.rsq[,2]>0.3]
sp.aff.rows=match(sp.aff,rownames(rda.ford.rare.hel.all.treats))
#^yielding NAs....why?
text(rda.ford.rare.hel.all.treats[sp.aff.rows,1:2],labels=rownames(rda.ford.rare.hel.all.treats[sp.aff.rows,]))
```
